...
import threading
import time
from WB_sRGB_Python.classes.WBsRGB import WBsRGB
import cv2
...
dataset_out = r'E:\datasets\Market-corrected-by-WB-sRGB'
cor_folder = [
    os.path.join(dataset_out, 'bounding_box_train_cor'),
    os.path.join(dataset_out, 'bounding_box_test_cor'), 
    os.path.join(dataset_out, 'query_cor')
]
    ...
    def run(self):
        ...
        for i, name in enumerate(self.imgs):
            if i % self.n_part == self.part:
                ...
                # Process
                I_in = cv2.imread(imgpath)
                I_out = wbModel.correctImage(I_in)
                cv2.imwrite(os.path.join(self.base_out, name), I_out * 255)
